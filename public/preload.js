...
const baseDir = path.join(appPath, './qiko-plugins-new');
...
window.qiko = {
  ...
  // 事件
  onPluginEnter(cb) {
    typeof cb === 'function' && (window.qiko.hooks.onPluginEnter = cb);
  },
  onPluginReady(cb) {
    typeof cb === 'function' && (window.qiko.hooks.onPluginReady = cb);
  },
  onPluginOut(cb) {
    typeof cb === 'function' && (window.qiko.hooks.onPluginOut = cb);
  },
  openPlugin(plugin) {
  ...
  setSubInput(onChange, placeholder = '', isFocus) {
    typeof onChange === 'function' &&
      (window.qiko.hooks.onSubInputChange = onChange);
  ...
  removeSubInput() {
    delete window.qiko.hooks.onSubInputChange;
  ...
  screenCapture(cb) {
    typeof cb === 'function' &&
      (window.qiko.hooks.onScreenCapture = ({ data }) => {
        cb(data);
      });
