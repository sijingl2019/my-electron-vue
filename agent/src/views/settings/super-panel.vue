<template>
  <p>{{ $t('agent.settings.superPanel.tips') }}</p>
        ...
        <a-list-item v-if="item">
          <template #actions>
            <a-button
              ...
              type="text"
            >
              {{ $t('agent.settings.superPanel.add') }}
            </a-button>
            <a-button
              v-else
              ...
              type="text"
            >
              {{ $t('agent.settings.superPanel.remove') }}
            </a-button>
          </template>
          <a-list-item-meta>
...
const localPlugins = computed(() =>
  store.state.localPlugins.filter(
    (plugin) =>
      ...
      plugin.name !== 'qiko-system-super-panel'
  )
);
...
const superPanelPlugins = ref(
  window.qiko.db.get('super-panel-db') || {
    data: [],
    _id: 'super-panel-db',
  }
);
...
const addPluginToSuperPanel = (plugin) => {
  ...
  window.qiko.db.put(toRaw(superPanelPlugins.value));
};
