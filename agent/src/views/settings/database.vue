...
const defaultConfig = window.qiko.dbStorage.getItem(
  'qiko-db-jg-webdav'
) || {
...
const showDetail = (key) => {
  show.value = true;
  detail.value = window.qiko.db.get(key);
};
...
const openHelp = () => {
  window.qiko.shellOpenExternal('https://help.jianguoyun.com/?p=2064');
};

const store = useStore();

const pluginsData = window.qiko.db.get('RUBICK_PLUGIN_INFO');
...
const dataPlugins = computed(() => {
  ...
  return pluginsData.data.map((item) => {
    let plugin = null;
    if (['qiko-system-feature'].includes(item.name)) {
      plugin = {
        pluginName: '主程序',
        isdownload: true,
    ...
    const data = item.keys.map((key) => window.qiko.db.get(key));
    return {
      ...item,
      plugin,
      data,
