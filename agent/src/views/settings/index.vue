<template>
  <div class="settings">
    <div class="view-title"> {{ $t('agent.settings.title') }}</div>
    <div class="view-container">
      <a-menu v-model:selectedKeys="currentSelect" mode="horizontal">
        <a-menu-item key="userInfo">
          {{ $t('agent.settings.account.accountInfo') }}
        </a-menu-item>
        <a-menu-item key="normal">
          {{ $t('agent.settings.basic.title') }}
        </a-menu-item>
        <a-menu-item key="localstart">
          {{ $t('agent.settings.localstart.title') }}
        </a-menu-item>
        <a-menu-item key="global">
          {{ $t('agent.settings.global.title') }}
        </a-menu-item>
        <!--        <a-menu-item key="superpanel">-->
        <!--          <template #icon>-->
        <!--            <FileAddOutlined />-->
        <!--          </template>-->
        <!--          {{ $t('agent.settings.superPanel.title') }}-->
        <!--        </a-menu-item>-->
        <a-menu-item key="database">
          {{ $t('agent.settings.database.title') }}
        </a-menu-item>
        <a-menu-item key="localhost">
          {{ $t('agent.settings.intranet.title') }}
        </a-menu-item>
      </a-menu>
      <div class="settings-detail">
        ...
        <div v-if="currentSelect[0] === 'normal'">
          <div class="setting-item">
            <div class="title">
              {{ $t('agent.settings.basic.shortcutKey') }}
            </div>
            <div class="settings-item-li">
              <div class="label">
                {{ $t('agent.settings.basic.showOrHiddle') }}
              </div>
            ...
            <div class="settings-item-li">
              <div class="label">
                {{ $t('agent.settings.basic.screenCapture') }}
              </div>
              <a-tooltip placement="top" trigger="click">
                <template #title>
                  <span>{{ tipText }}</span>
          ...
          <div class="setting-item">
            <div class="title">{{ $t('agent.settings.basic.common') }}</div>
            <div class="settings-item-li">
              <div class="label">
                {{ $t('agent.settings.basic.autoPaste') }}
              </div>
              <a-switch
                v-model:checked="common.autoPast"
                :checked-children="$t('agent.settings.basic.on')"
                :un-checked-children="$t('agent.settings.basic.off')"
              ></a-switch>
            </div>
            <div class="settings-item-li">
              <div class="label">{{ $t('agent.settings.basic.autoBoot') }}</div>
              <a-switch
            ...
            <div class="settings-item-li">
              <div class="label">
                {{ $t('agent.settings.basic.spaceExec') }}
              </div>
              <a-switch
            ...
            <div class="settings-item-li">
              <div class="label">
                {{ $t('agent.settings.basic.history') }}
              </div>
              <a-switch
          ...
          <div class="setting-item">
            <div class="title">{{ $t('agent.settings.basic.theme') }}</div>
            <div class="settings-item-li">
              <div class="label">{{ $t('agent.settings.basic.darkMode') }}</div>
              <a-switch
          ...
          <div class="setting-item">
            <div class="title">{{ $t('agent.settings.basic.language') }}</div>
            <div class="settings-item-li">
              <div class="label">
                {{ $t('agent.settings.basic.changeLang') }}
              </div>
              <a-select
        ...
        <div v-if="currentSelect[0] === 'global'">
          <a-collapse>
            <a-collapse-panel
              key="1"
              :header="$t('agent.settings.global.instructions')"
            >
              <div>
                {{ $t('agent.settings.global.tips') }}
              </div>
              <h3 style="margin-top: 10px">
                {{ $t('agent.settings.global.example') }}
              </h3>
          ...
          <div class="feature-container">
            <div class="keywords item">
              <div>{{ $t('agent.settings.global.shortcutKey') }}</div>
            ...
            <div class="short-cut item">
              <div>{{ $t('agent.settings.global.funtionKey') }}</div>
          ...
          <div @click="addConfig" class="add-global">
            <PlusCircleOutlined />
            {{ $t('agent.settings.global.addShortcutKey') }}
          </div>
        </div>
...
const examples = [
  {
    title: t('agent.settings.global.example1'),
    desc: t('agent.settings.global.tips1'),
  },
  {
    title: t('agent.settings.global.example2'),
    desc: t('agent.settings.global.tips2'),
  },
];
...
const tipText = computed(() => {
  ...
  return t('agent.settings.global.addShortcutKeyTips', {
...
const options = ref([
  {
    value: 'zh-CN',
    label: t('agent.settings.basic.cn'),
  },
  {
    value: 'en-US',
    label: t('agent.settings.basic.en'),
  },
]);
