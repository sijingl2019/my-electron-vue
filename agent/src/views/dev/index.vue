<template>
  <div class="dev">
    <div class="view-title">{{ $t('agent.dev.title') }}</div>
    <div class="view-container">
      <a-alert
        style="margin-bottom: 40px"
        :message="$t('agent.dev.tips')"
        type="warning"
      />
      <a-form
        ref="formRef"
      ...
      >
        <a-form-item :label="$t('agent.dev.pluginName')" name="name">
...
const downloadPlugin = async (pluginName) => {
  ...
  message.success(t('agent.dev.installSuccess', { pluginName: pluginName }));
  loading.value = false;
};
...
const refresh = () => {
  formRef.value.validate().then(() => {
    ...
    message.success(
      t('agent.dev.refreshSuccess', { pluginName: formState.name })
    );
  });
};
