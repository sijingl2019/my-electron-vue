...
export default () => {
  ...
  const viewReadyFn = async (window, { pluginSetting, ext }) => {
    ...
    darkMode &&
      view.webContents.executeJavaScript(
        `document.body.classList.add("dark");window.qiko.theme="dark"`
      );
  ...
  const createView = (plugin, window: BrowserWindow) => {
    ...
    // 再尝试去找
    if (plugin.name === 'qiko-system-feature' && !pluginIndexPath) {
  ...
  const removeView = (window: BrowserWindow) => {
    ...
    setTimeout(() => {
      ...
      window.webContents?.executeJavaScript(`window.initQiko()`);
      view = undefined;
    }, 0);
  };
  ...
  const executeHooks = (hook, data) => {
    if (!view) return;
    const evalJs = `if(window.qiko && window.qiko.hooks && typeof window.qiko.hooks.on${hook} === 'function' ) {
          try {
            window.qiko.hooks.on${hook}(${data ? JSON.stringify(data) : ''});
          } catch(e) {}
        }
      `;
