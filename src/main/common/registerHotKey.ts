...
const registerHotKey = (mainWindow: BrowserWindow): void => {
  ...
  const setTheme = async () => {
    mainWindow.webContents.executeJavaScript(`window.qiko.changeTheme()`);
    mainWindow.getBrowserViews().forEach((view: BrowserView) => {
      view.webContents.executeJavaScript(`window.qiko.changeTheme()`);
    });
  };
  ...
  const setDarkMode = async () => {
    ...
    if (isDark) {
      ...
      mainWindow.webContents.executeJavaScript(
        `document.body.classList.remove("dark");window.qiko.theme="light"`
      );
      mainWindow.getBrowserViews().forEach((view: BrowserView) => {
        view.webContents.executeJavaScript(
          `document.body.classList.remove("dark");window.qiko.theme="light"`
  ...
  const init = async () => {
    ...
    globalShortcut.register(config.perf.shortCut.quit, () => {
      // mainWindow.webContents.send('init-qiko');
      // mainWindow.show();
    });
