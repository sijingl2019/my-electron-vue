...
const getSearchFiles = (argv = process.argv, cwd = process.cwd()) => {
  const files = argv.slice(2); // 过滤['qiko.exe', 'search']这两个参数，直接获取需要上传的图片路径
  let result: any = [];
  if (files.length > 0) {
    // 如果图片列表不为空
    result = files
...
const putFileToQiko = (webContents, files) => {
...
const macBeforeOpen = () => {
  const dest = `${os.homedir}/Library/Services/qiko.workflow`;
  if (fs.existsSync(dest)) {
    ...
    try {
      copyFileOutsideOfElectronAsar(
        path.join(__static, 'qiko.workflow'),
        dest
      );
    } catch (e) {
      console.log(e);
...
export { getSearchFiles, putFileToQiko, macBeforeOpen };
