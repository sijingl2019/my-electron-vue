<template>
  <div class="qiko-select">
    ...
    <div v-else :class="currentPlugin.cmd ? 'qiko-tag' : ''">
      <img
        @click="() => emit('openMenu')"
        class="qiko-logo"
...
const changeValue = (e) => {
  // if (props.currentPlugin.name === 'qiko-system-feature') return;
...
const openDevTool = () => {
  ipcRenderer.send('msg-trigger', { type: 'openMainDevTools' });
};
