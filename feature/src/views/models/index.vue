...
const baseDir = path.join(appPath, './qiko-plugins');
...
const localPlugins = computed(() =>
  store.state.localPlugins.filter(
    (plugin) => plugin.name !== 'qiko-system-feature'
  )
);
...
const superPanelPlugins = ref(
  window.qiko.db.get('super-panel-user-plugins') || {
    data: [],
...
const addCmdToSuperPanel = ({ cmd, code }) => {
  ...
  const { rev } = window.qiko.db.put(JSON.parse(JSON.stringify(superPanelPlugins.value)));
...
const removePluginToSuperPanel = ({ cmd, name }) => {
  ...
  const { rev } = window.qiko.db.put(toRaw(superPanelPlugins.value));
  superPanelPlugins.value._rev = rev;
};
